...
together by the following equation: $$\boldsymbol{F}= \frac{d\boldsymbol{p}}{dt}$$
...
$$ Here, $\boldsymbol{p}$ is the linear momentum of some
...
$\boldsymbol{r}$ is defined as: $$\
    \boldsymbol{\tau}= \boldsymbol{r}\times\boldsymbol{F}
$$ Yet again, our choice of origin affects the value of
...
$$ The path traced by this particle when revolving around
a fixed axis will always be circular, regardless of the object's shape.
So, by circular motion, we know that: $$\
    \boldsymbol{v}_j = \boldsymbol{\omega}\times\boldsymbol{r}_j
$$ The angular velocity points along the z-axis.
...
$$ Thus, the total angular momentum of the body about the
z-axis with an origin aligning with this axis is: $$\
    L_z = \sum_j m_j\rho_j^2\omega = \omega\sum_j m_j\rho_j^2
$$ The summation term is called the moment of inertia of
the body. It is defined by: $$\
    I_{body} = \sum_jm_j\rho_j^2
$$ Then: $$\
    L_z = I_{body}\omega
$$ Naturally, the moment of inertia would be solved using
using an integral. A more useful definition would be: $$\
    I = \int\rho^2dm
$$ This can be calculated using some basic calculus for a
variety of different objects. The dynamic consequence of this is
straightforward: $$\
    \boldsymbol{\tau}= \frac{d}{dt}(I\omega) = I\alpha
$$ Here, $\alpha$ is the angular acceleration of the body.
...
$$ Here, $\dot{\boldsymbol{r}}_j'$ is the position of the
particle relative to the COM. Next: $$\
...
$$ One half of the angular momentum comes from the
...
$$ Relating (3.11) with (3.9) using (1.5) we get:
$$\
...
$$ This proves that center of mass's motion is irrelevant
and that: $$\
    \boldsymbol{\tau}_{COM} = \frac{d\boldsymbol{L}_{COM}}{dt}
$$ The angular momentum about a body's COM was:
$$\
...
$$ These cross products can be computed and each component
...
$$ You may already see where this is going. The angular
...
$$ The middle term cancels out by the fact that
$\sum m_jv_j'$ is zero. This yields the formula: $$\
    T=\frac{1}{2}MV^2 + \frac{1}{2}\sum m_j\left(v_j'\right)^2
$$ Next, let's process the rotational kinetic energy term:
$$\
...
$$ A linear algebra trick can be used to convert this to:
$$\
...
$$ For any body, it is possible to find a set of
...
the inertia tensor becomes diagonal: $$\
    T_{rot} = \frac{1}{2}
    \begin{bmatrix}
        w_x\\
        w_y\\
...
$$ Taking the derivative of this in the inertial frame
yields: $$\
...
$$ The LHS is simply the derivative in the inertial frame.
...
$$ $$\
    ...
    \end{bmatrix} = \begin{bmatrix}
        ...
        \tau_z
    \end{bmatrix}
$$ The results are the Euler Rigid Body Equations. We
...
$$ Note that these equations are in the rigid body frame.
dd
